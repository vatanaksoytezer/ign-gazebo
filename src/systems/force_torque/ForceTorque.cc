...
/// \brief Private Force-Torque data class.
class ignition::gazebo::systems::ForceTorquePrivate
{
  /// \brief A map of FORCE_TORQUE entity to its FORCE_TORQUE sensor.
  ...
  /// \brief Update Force-Torque sensor data based on physics data
  /// \param[in] _ecm Immutable reference to ECM.
  public: void Update(const EntityComponentManager &_ecm);

  /// \brief Remove Force-Torque sensors if their entities have been removed from
  /// simulation.
  /// \param[in] _ecm Immutable reference to ECM.
...
void ForceTorquePrivate::CreateForceTorqueEntities(EntityComponentManager &_ecm)
{
  ...
  _ecm.EachNew<components::ForceTorque, components::ParentEntity>(
    [&](const Entity &_entity,
        const components::ForceTorque *_force_torque,
        const components::ParentEntity *_parent)->bool
      {
        // create sensor
        std::string sensorScopedName =
            removeParentScope(scopedName(_entity, _ecm, "::", false), "::");
        sdf::Sensor data = _force_torque->Data();
...
void ForceTorquePrivate::Update(const EntityComponentManager &_ecm)
{
  ...
  _ecm.Each<components::ForceTorque,
    ...
    [&](const Entity &_entity,
        const components::ForceTorque * /*_force_torque*/,
        const components::Force *_force,
        const components::Torque *_torque)->bool
      {
        auto it = this->entitySensorMap.find(_entity);
        if (it != this->entitySensorMap.end())
        {
          const auto &forceTorqueWorldPose = _worldPose->Data();
          it->second->SetWorldPose(forceTorqueWorldPose);

          // Set the FT Sensor - force (defined in Force Torque's local frame)
          it->second->SetForce(_force->Data());

          // Set the FT Sensor - torque (defined in Force Torque's local frame)
          it->second->SetTorque(_torque->Data());
         }
        else
        {
  ...
  _ecm.EachRemoved<components::ForceTorque>(
    [&](const Entity &_entity,
        const components::ForceTorque *)->bool
      {
        ...
        {
          ignerr << "Internal error, missing Force Torque sensor for entity ["
                 << _entity << "]" << std::endl;
          return true;
        }
